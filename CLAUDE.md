...
Firmware + tooling for a WiFi/IP megaphone speaker built on the Waveshare ESP32-S3-AUDIO-Board (Xtensa dual-core, 8MB Octal PSRAM, 16MB flash, ES8311 DAC + ES7210 ADC + TCA9555 I/O expander). A separate Docker service (not yet implemented, see `docker/`) receives alarm JSON, synthesizes speech with Piper TTS, and streams the audio in real time over UDP to one or more of these speakers (one per población/zona in the BRAVO 2 deployment).

Repo layout:
- `esp32s3-firmware/ip-speaker/` — the ESP-IDF firmware project (all the C code lives in `main/`).
- `docker/` — reference/tooling for the sender side (currently just `reference_send_audio.py`; the actual Docker service is a separate future project).
...
6. `http_status_server` (STA mode only) exposes `GET /status` (firmware version, IP, RSSI, `idle`/`streaming` state — driven by `udp_audio_server_is_streaming()`, not buffer occupancy — volume, timestamps, uptime) and `POST /volume`. `time_sync` provides the timestamps via SNTP (`pool.ntp.org`, Europe/Madrid TZ).
